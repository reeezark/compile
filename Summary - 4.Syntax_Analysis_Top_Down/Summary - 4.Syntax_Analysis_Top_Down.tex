...
\node[n,right=0.95cm of type] (td) {Top-down};
\node[n,right=0.95cm of td] (rdp) {递归下降\\RDP};
\node[hi,below=0.8cm of rdp] (problem) {左递归 / 回溯};
\node[hi,right=0.95cm of rdp] (pred) {预测分析\\Predictive};
\node[n,right=0.95cm of pred] (ff) {FIRST / FOLLOW};
\node[n,right=0.95cm of ff] (ll) {LL(1) 表};
\draw[arr] (type) -- (td);
\draw[arr] (td) -- (rdp);
