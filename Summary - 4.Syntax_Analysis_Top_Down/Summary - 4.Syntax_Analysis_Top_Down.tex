...
\node[n,right=1.5cm of type] (td) {Top-down};
\node[n,right=1.5cm of td] (rdp) {递归下降\\RDP};
\node[hi,below=0.8cm of rdp] (problem) {左递归 / 回溯};
\node[hi,right=1.5cm of rdp] (pred) {预测分析\\Predictive};
\node[n,right=1.5cm of pred] (ff) {FIRST / FOLLOW};
\node[n,right=1.5cm of ff] (ll) {LL(1) 表};
\draw[arr] (type) -- (td);
\draw[arr] (td) -- (rdp);
...
\subsection{计算 FIRST(\(\alpha\)) 的规则 \pg{34,46--48}}
...
\begin{longtable}{p{0.18\linewidth}p{0.28\linewidth}p{0.27\linewidth}p{0.2\linewidth}}
\toprule
已匹配 & 栈 & 剩余输入 & 动作 \\
\midrule
\endfirsthead
...
由这些集合可得到前面使用的预测分析表。注意 \(E'\to\eps\) 填到 \(M[E',)]\) 和 \(M[E',\$]\)，因为 \(\mathrm{FOLLOW}(E')=\{\$,)\}\)；\(T'\to\eps\) 填到 \(M[T',+]\)、\(M[T',)]\)、\(M[T',\$]\)，因为这些符号都在 \(\mathrm{FOLLOW}(T')\) 中。
